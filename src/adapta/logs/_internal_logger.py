...
class _InternalLogger(LoggerInterface, ABC):
    ...
    def _activate_redirect(
        ...
        tmp_symlink_stdout = os.path.join(tempfile.gettempdir(), f"{uuid.uuid4()!s}-out").encode("utf-8")
        tmp_symlink_stderr = os.path.join(tempfile.gettempdir(), f"{uuid.uuid4()!s}-err").encode("utf-8")
