...
class DatadogMetricsProvider(MetricsProvider):
    ...
    def __init__(self, metric_namespace: str, fixed_tags: dict[str, str] | None = None, debug=False, **options):
        self._options = {
    ...
    @classmethod
    def udp(cls, metric_namespace: str, fixed_tags: dict[str, str] | None = None, debug=False):
        """
    ...
    def count(self, metric_name: str, metric_value: int, tags: dict[str, str] | None = None) -> None:
        statsd.increment(metric=metric_name, value=float(metric_value), tags=convert_datadog_tags(tags))
