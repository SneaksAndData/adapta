...
from adapta.dataclass_validation.validation.validation_abstract import (
    AbstractValidationClass,
)
    ...
    def _validate_primary_keys(self, **kwargs) -> None:
        ...
        if primary_keys and len(self._data) != len(self._data.select(primary_keys).unique()):
            self._failed_validations += [
                f"Duplicated primary key(s) found. Please ensure primary key(s) are unique. This is the provided primary key(s): {primary_keys}",
            ]
