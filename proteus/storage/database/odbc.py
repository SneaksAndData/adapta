...
class OdbcClient(ABC):
    ...
    def materialize(
        self, data: pandas.DataFrame, schema: str, name: str, overwrite: bool = False, chunksize: Optional[int] = None
    ) -> Optional[int]:
        """
        ...
        try:
            ...
            self._logger.error(
                "Error while materializing a dataframe into {schema}.{table}", schema=schema, table=name, exception=ex
            )
            return None
        finally:
            ...
            if active_tran and active_tran.is_active:
                self._logger.debug(
                    "Found an active transaction for {schema}.{table}. Committing it.", schema=schema, table=name
                )
                active_tran.commit()
