...
class DataDogApiHandler(Handler):
    ...
    def _attach_interrupt_handlers(self) -> None:
        ...
        self._existing_sigterm_handler: Callable = signal.getsignal(signal.SIGTERM) if callable(signal.getsignal(signal.SIGTERM)) else None
        self._existing_sigint_handler: Callable = signal.getsignal(signal.SIGINT) if callable(signal.getsignal(signal.SIGINT)) else None
